...
describe("1_object_metadata", () => {
  ...
  describe("isApplicable", () => {
    it("should not want to apply on empty database", async () => {
    ...
    it("should want to apply on database with checkpoints collection but no schema version collection", async () => {
      const db = client.db(dbName);
      await db.dropDatabase();
  ...
  describe("apply", () => {
    let expectedCheckpoints: Record<
      string,
      {
    ...
    beforeEach(async () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        const [__, serializedMetadata] = serde.dumpsTyped(metadata);
